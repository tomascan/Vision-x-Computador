function precision=test(name_dir, check_file)
...
for num_image=3:length(directorio)
    ...
    tabla = struct2table(gusanos_props);
    %disp(tabla(:, {'Area'}));
    ...
    for i = 1:num_gusanos
        if gusanos_props(i).Area > 1700
            ...
            % Añadir el gusano procesado a la máscara
    ...
    if (num_vivos > num_muertos)                                                % Es comprova si hi ha més cucs vius que morts.
    ...
    total_img=total_img+1;                                              % S'incrementa el nombre d'imatges tractades.
end
precision=(correct_img/total_img)*100;                            % Per calcular la precisió, es divideix el nombre d'imatges clasificades correctament entre el nombre d'imatges tractades.
end